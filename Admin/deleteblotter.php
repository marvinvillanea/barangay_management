...
$get_record = mysqli_query($connection, "SELECT * FROM blotter WHERE idblotter='$prk_id'");
$get_record_num = mysqli_num_rows($get_record);
    ...
    if ($get_record_num > 0 ){
        ...
        while ($i = mysqli_fetch_assoc($get_record)){
             ...
             $db_prk_no = $i["full_name"];
             $db_prk_leader = $i['reason'];
       ...
       $query_delete =  mysqli_query($connection, "DELETE FROM blotter WHERE idblotter = '$prk_id' ");
        
        if ($query_delete === TRUE){
          echo "<script> alert('has been successfully Deleted');window.location='blotter.php';</script>";
        }else {
...
<body class="bg-dark">
  ...
  <div class="container">
    <div class="card card-register mx-auto mt-5">
      <div class="card-header">Delete Blotter</div>
      <div class="card-body">
        <form method = "POST">
